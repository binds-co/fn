...
describe('healthScore', () => {
  ...
  describe('getTranslatedChurnStatusLabel', () => {
    it('should return Muito Baixa for churn 0', () => {
      expect(getTranslatedChurnStatusLabel(0, false)).toBe(CHURN.LABELS.SUPER_LOW);
    });

    it('should return Baixa for churn 20', () => {
      expect(getTranslatedChurnStatusLabel(20, false)).toBe(CHURN.LABELS.LOW);
    });

    it('should return Média for churn 40', () => {
      expect(getTranslatedChurnStatusLabel(40, false)).toBe(CHURN.LABELS.MEDIUM);
    });

    it('should return Alta for churn 60', () => {
      expect(getTranslatedChurnStatusLabel(60, false)).toBe(CHURN.LABELS.HIGH);
    });

    it('should return Muito Alta for churn 80', () => {
      expect(getTranslatedChurnStatusLabel(80, false)).toBe(CHURN.LABELS.SUPER_HIGH);
    });

    it('should return Invalid value for churn -1', () => {
      expect(getTranslatedChurnStatusLabel(-1, false)).toBe('Invalid value');
    });
  });
});
